...
class AddClassifyPostPage extends StatefulWidget {

  String title;
  int cid;
  int ciid;
  AddClassifyPostPage(this.title,this.cid,this.ciid);
...
class _PageState extends State<AddClassifyPostPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          title: Text(
            widget.title != null?widget.title:'分类发贴',
          ),
          centerTitle: true,
          actions: <Widget>[
            FlatButton.icon(
...
//    print(photos);
  ...
  void _reqAddClassifyPost(title, content, extend) {
    ...
    CHttp.post(
        CHttp.DISCUSS_ADD_CLASSIFY,
        (data) {
          Navigator.pop(context);
          ToastUtil.showToast(context, '发布成功');
          Navigator.pop(context, PostModel(data));
        },
        params: PAddClassifyPost(Provider.of<App>(context, listen: false).userid,0,1,widget.ciid, title,
                content,extend)
            .toJson(),
        errorCallback: (err) {
          Navigator.pop(context);
